...
#     count = 0
#     for x in range(len(game.state.board.xy)):
#         if game.state.board.xy[-1][x] is not None and game.state.board.xy[-1][x].iron > 1:
#             game.state.board.xy[-1][x].iron -= 1
#         else:
#             game.state.board.xy[-1][x] = None
#             count += 1
#     pull_down(game)
#     game.state.score += count
#     game.state.last_event = (game.state.play_time, f'+ {count}')
