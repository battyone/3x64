...
def add_points(game, blocks: [Block]):
    game.state.score += len(blocks)

def delete_blocks(game, blocks: [Block]):
    for (_, x, y) in blocks:
        game.state.board.xy[y][x] = None

def check_collisions(game):
    blocks : [Item] = list(get_blocks_to_delete(game.state.board.xy))
    if blocks:
        game.events.call('found_collisions', blocks)

def get_blocks_to_delete(board):
    for sequence in get_all_sequences(board):
